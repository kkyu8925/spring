...
public class GetAccStatController {
	...
	public String getAccStatForJSON(HttpServletRequest request, HttpServletResponse response, ModelMap model)
		...
		// OpenAPI 호출을 위한 피라미터 저장
		AccStatDTO pDTO = new AccStatDTO();
		pDTO.setUrl(url);

		// JSON으로부터 받은 결과를 바자에서 처리가능한 데이터 구조로 변경
	...
	public Map<String, Object> getAccStatNightForJSON(HttpServletRequest request, HttpServletResponse response)
		...
		// OpenAPI룔 피라미터 받아오기
		String yyyymm = CmmUtil.nvl(request.getParameter("yyyymm")); // 사고 년월
		String a_code = CmmUtil.nvl(request.getParameter("a_code")); // 사고 년월
		...
		// OpenAPI 호출을 위한 피라미터 저장하기
		AccStatDTO pDTO = new AccStatDTO();
		pDTO.setUrl(url);

		// JSON으로부터 받은 결과르 자바에서 처리가능한 데이터 구조로 변경
	...
	public String accStatForAjax(HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
